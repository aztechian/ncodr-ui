...
export default {
  ...
  data() {
    return {
      ...
      valid: false,
      inputLoading: false,
      typevalidation: [v => !!v || 'Type is required'],
      inputvalidation: [v => !!v || 'Input is required'],
      optionsValidation: [v => !!JSON.parse(v) || 'Not valid JSON'],
      inputFiles: [],
      search: null,
    };
  },
  ...
  methods: {
    ...
    getFiles(val) {
      ...
      return this.$store
        .dispatch('getQueueFiles', { queuename, val })
        .then((response) => {
